...
class DataTransforms(object):
    """
    Basic Data Transforms for MGCA.
...
class DetectionDataTransforms(object):
    """
    Data Transforms for Object Detection Tasks.
    
    Minimal transforms for detection since:
    - Bounding boxes need to be transformed alongside images
    - Strong spatial augmentation can harm detection performance
    - Color augmentation is safer (doesn't affect box coordinates)
    
    Note: The color jitter is defined but not currently applied
    (transforms only include ToTensor and Normalize).
    
    Args:
        is_train (bool): Whether training mode
        crop_size (int): Not used (kept for interface consistency)
        jitter_strength (float): Color jitter intensity multiplier
        
    Color Jitter Parameters (when applied):
    - Brightness: ±80% * jitter_strength
    - Contrast: ±80% * jitter_strength
    - Saturation: ±80% * jitter_strength
    - Hue: ±20% * jitter_strength
    """
    
    def __init__(self, is_train: bool = True, crop_size: int = 224, jitter_strength: float = 1.):
        if is_train:
            # Define color jitter (can be used for color augmentation)
            # Currently not applied in the pipeline below
            self.color_jitter = transforms.ColorJitter(
                0.8 * jitter_strength,  # Brightness
                0.8 * jitter_strength,  # Contrast
                0.8 * jitter_strength,  # Saturation
                0.2 * jitter_strength,  # Hue
            )

            # Gaussian blur kernel size (10% of image size, must be odd)
            kernel_size = int(0.1 * 224)
            if kernel_size % 2 == 0:
                kernel_size += 1

            # Only ToTensor and Normalize (no spatial transforms)
            # Spatial transforms would require box coordinate adjustment
            data_transforms = [
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
            ]
        else:
            # Same for validation (minimal processing)
            data_transforms = [
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
            ]
